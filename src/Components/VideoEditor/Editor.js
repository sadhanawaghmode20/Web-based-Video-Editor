...
function Editor({ videoUrl, timings, setTimings }) {
	...
	useEffect(() => {		
		if (playVideoRef.current && timings && timings.length > 0) {
			const firstTiming = timings[0];
			if (firstTiming && 'start' in firstTiming && 'end' in firstTiming) {
				const seek = (playVideoRef.current.currentTime - firstTiming.start) / playVideoRef.current.duration * 100;
				setSeekerBar(seek);
				...
				console.error("Invalid timing object or missing start/end properties");
			}
		} else {
			console.error("playVideoRef.current is not properly defined or timings array is empty or not defined");
		}
	...
	const time = timings;
	if (playVideoRef && playVideoRef.current) {
		playVideoRef.current.onloadedmetadata = () => {
			console.log("playVideoRef.current:", playVideoRef.current); // Log current value of playVideoRef.current
			if (playVideoRef.current) {
				if (playVideoRef.current.duration !== null) {
					if (time.length === 0) {
					...
					console.error("playVideoRef.current.duration is null");
				}
			} else {
				console.error("playVideoRef.current is null");
			}
		};
	} else {
		console.error("playVideoRef is not properly initialized or playVideoRef.current is null");
	}
	...
	const playPause = () => {
		if (playing) {
			...
			if (timings && timings.length > 0 && playVideoRef.current) {
				const firstTiming = timings[0];
				if (firstTiming && 'end' in firstTiming) {
					if (playVideoRef.current.currentTime >= firstTiming.end) {
						playVideoRef.current.pause();
						setPlaying(false);
						currentlyGrabbedRef.current = { index: 0, type: 'start' };
						playVideoRef.current.currentTime = firstTiming.start;
						progressBarRef.current.style.left = `${firstTiming.start / playVideoRef.current.duration * 100}%`;
						progressBarRef.current.style.width = '0%';
					}
				} else {
					console.error("Invalid timing object or missing end property");
				}
			} else {
				console.error("timings array is not properly defined or empty, or playVideoRef is not defined");
			}
